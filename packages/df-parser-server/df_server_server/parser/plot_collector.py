...
class PlotCollector(m.MatcherDecoratableVisitor):
    ...
    def are_keys_valid(self, node: cst.DictElement):
        ...
        is_all_valid = all(
            m.matches(
                elem,
                m.OneOf(self.response, self.transitions, self.processing, self.misc),
            )
        ...
        if not is_all_valid:
            raise Exception(
                "All keys should be one of `RESPONSE`, `TRANSITIONS`, `PROCESSING`, `MISC`"
            )
                ...
                if m.matches(elem.key, m.TypeOf(m.Tuple)):
                    ...
                    if len_t >= 2 and len_t <= 3:
                        check_type_flow_node = any(
                            ...
                            if not m.matches(
                                tuple_elem.value, m.TypeOf(m.SimpleString | m.Integer)
                            )
                        )
                        if check_type_flow_node:
                            raise Exception(
                                f"Transition ({elem.key.elements[:-1]}) has incorrect structure!"
                            )
                        if len_t == 3:
                            check_priority_type = m.matches(
                                elem.key.elements[-1].value,
                                m.TypeOf(m.Integer | m.Float),
                            )
                            if not check_priority_type:
                                raise Exception(
                                    f"Priority {elem.key.elements[-1]} is not a number!"
                                )
                    else:
                        raise Exception(
                            f"Transition to {elem.key} has incorrect structure!"
                        )
                transition_condition = m.matches(
                    elem.value, m.TypeOf(m.Name | m.Call | m.Attribute)
                )
                if not transition_condition:
                    raise Exception(
                        f"Transition to {elem.key} by condition {elem.value} has wrong type!"
                    )
        else:
    ...
    def check_procs_structure(self, node: cst.DictElement):
        ...
        if m.matches(node.value, m.TypeOf(m.Dict)):
            for elem in node.value.elements:
                key_condition = m.matches(
                    elem.key, m.TypeOf(m.Integer | m.Float | m.SimpleString)
                )
                if not key_condition:
                ...
                if not proc_condition:
                    raise Exception(
                        f"Processing key {elem.key} has wrong value {elem.value}"
                    )
        elif m.matches(node.value, m.TypeOf(self.func_type)) and not m.matches(
            node.value, self.bool_type
        ):
            pass
        else:
            raise Exception(
                f"Processing {node.value} has incorrect type {type(node.value)}!"
            )

    @m.call_if_not_inside(global_)
    def check_flow_structure(self, node):
        # if this is flow, this should be DictElement with str key
        if not m.matches(node, m.TypeOf(m.DictElement)) and not m.matches(
            node.key, m.TypeOf(m.SimpleString | m.Name)
        ):
            return False
    ...
    def visit_Assign(self, node):
        ...
        if global_condition > 1:
            raise Exception(
                f"Only one `GLOBAL` can be used! ({global_condition} found)"
            )
    ...
    def parse_args_kwargs(self, func_args: list):
        ...
        for arg in func_args:
            ...
            if m.matches(arg, m.TypeOf(m.ListComp)):
                ...
                res = {
                    "output": self.parse_args_kwargs(arg.elements),
                    "type": str(type(arg)),
                }
                ...
                arg, m.TypeOf(m.SimpleString | m.Integer | m.Float)
            ) or m.matches(arg, self.bool_type):
                res = arg.value
        ...
        for transition in transitions:
            ...
            if m.matches(transition.key, m.TypeOf(self.func_type)):
                ...
                transition_dict["dest"] = {
                    "flow": current_flow,
                    "node": transition.key.value.replace('"', ""),
                }
                ...
                if len(transition.key.elements) == 3:
                    transition_dict["dest"]["priority"] = transition.key.elements[
                        2
                    ].value.value.replace('"', "")
            ...
            if m.matches(transition.value, m.TypeOf(self.func_type)):
                ...
                raise Exception(
                    f"Condition should be Callable! (got {type(transition.value)})"
                )
    ...
    def process_call(self, node):
        if not m.matches(node, m.TypeOf(self.func_type)):
            raise Exception(
                f"Function call is incorrect: node should be some function! (got {type(node)})"
            )
    ...
    def generate_processing(self, processing):
        ...
        if m.matches(processing, m.TypeOf(m.Dict)):
            for dict_element in processing.elements:
                ...
                proc_seq["proc_object"].append(
                    {"proc_name": key, "proc_object": proc_object}
                )
        else:
            proc_object = self.process_call(processing)
            proc_seq["proc_object"].append(
                {"proc_object": proc_object, "proc_id": get_processing_id()}
            )
        return proc_seq
    ...
    def parse_node(self, node, flow_name=None):
        ...
        for elem in node.value.elements:
            if m.matches(elem, self.response):
                ...
                transitions_object = self.generate_transitions(
                    elem.value.elements, current_flow=flow_name
                )
