...
def parse_file(drawio_fn):
    ...
    for node in elems:
        if "isnode" in node.attrib:
            ...
            old_titles = form_data.get(
                "old_titles", [unesc(node.attrib["old_title"]), None]
            )
            if len(old_titles) > 1:
            ...
            try:
                sys.stderr.write(
                    f"target: "
                    + str(
                        doc.xpath(f"//*[@source='{int(node.attrib['target'])}']")[
                            0
                        ].attrib
                    )
                    + "\n"
                )
                realtarget = doc.xpath(f"//*[@source='{int(node.attrib['target'])}']")[
                    0
                ].attrib["target"]
                sys.stderr.write(
                    f'EDGE: {node} {int(node.attrib["source"])} -> {int(realtarget)} : {unesc(node.attrib["reallabel"])}\n'
                )
                edges[int(node.attrib["source"])][int(realtarget)] = unesc(
...
def get_updated_nodes(nodes, edges):
    ...
    for node_dict in nodes.values():
        ...
        if sfc != "":
            updated[flow_name][node_key]["MISC"]['"speech_functions"'] = ListUpdate(
                [sfc], order_significant=True, reverse=True
            )
        if midas != "":
    ...
    for node_dict in nodes.values():
        ...
        for edge, edge_title in edges[int(node.attrib["parent"])].items():
            ...
            sys.stderr.write(
                f"node: {node_name}, flow name {node.attrib['flow']}, target flow {nodes[edge]['node'].attrib['flow']}\n"
            )
            if unesc(node.attrib["flow"]) != unesc(nodes[edge]["node"].attrib["flow"]):
                ...
                old_target_node = next(
                    (o for o, n in renames.items() if n == new_target_node),
                    new_target_node,
                )
                sys.stderr.write(
                    f"flow name {target_flow}, node name (new) {new_target_node}, (old) {old_target_node}\n"
                )
                ...
                old_name = next(
                    (o for o, n in renames.items() if n == new_name), new_name
                )
                val = edge_title
                transitions[KeyUpdate(old_key=old_name, new_key=new_name)] = val
